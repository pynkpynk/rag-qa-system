...
from app.core.authz import effective_auth_mode, demo_owner_sub_from_token  # noqa: E402
...
def _ensure_seed_document(
    db: Session, owner_sub: str, spec: dict[str, str]
) -> Document:
    doc = (
        db.query(Document).filter(Document.content_hash == spec["content_hash"]).first()
    )
    if doc:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Seed a dev/demo run with attached documents."
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Allow running outside local/dev/demo modes",
    )
    parser.add_argument(
        "--demo-token",
        dest="demo_token",
        help="Plaintext demo token (hashes to derive owner_sub in demo mode)",
    )
    args = parser.parse_args(argv)

    mode = effective_auth_mode()
    app_env = (os.getenv("APP_ENV") or "dev").strip().lower()
    non_prod_env = app_env not in {"prod", "production"}
    allowed_mode = mode == "dev" or (non_prod_env and mode == "demo")
    if not allowed_mode and not args.force:
        print(
            "[dev_seed_run] AUTH_MODE must be 'dev' (or 'demo' when APP_ENV != prod). Pass --force to override.",
            file=sys.stderr,
        )
        return 1
    if app_env in {"prod", "production"} and not args.force:
        print(
            "[dev_seed_run] Refusing to run in prod without --force.", file=sys.stderr
        )
        return 1
    ...
    if mode == "demo":
        demo_token = (
            args.demo_token or os.getenv("DEMO_TOKEN_PLAINTEXT") or ""
        ).strip()
        if not demo_token:
            print(
                "[dev_seed_run] DEMO_TOKEN_PLAINTEXT env or --demo-token is required when AUTH_MODE=demo.",
                file=sys.stderr,
            )
            return 1
        owner_sub = demo_owner_sub_from_token(demo_token)
    if not owner_sub:
        print(
            "[dev_seed_run] DEV_SUB is empty; update backend/.env.local.",
            file=sys.stderr,
        )
        return 1
